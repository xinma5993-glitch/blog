...
浏览器中新建的文章会直接跳转到阅读页并进入编辑模式，也会出现在首页文章列表中。阅读页通过 `post.html?slug=文章标识` 打开，支持直接编辑标题、正文、分类、标签和摘要。

在阅读页编辑正文时，可以把图片直接粘贴到正文区域。图片会以浏览器本地数据形式保存，太大的图片可能导致保存失败，建议先压缩到 800KB 以内。

这些动态文章依赖当前浏览器保存的数据；如果换浏览器或清除缓存，需要先导入之前导出的 JSON。
